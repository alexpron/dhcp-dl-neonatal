...
def save_gifti_surface(vert, face, save_dir, surf_hemi="left", surf_type="wm"):
    ...
    # meta data
    vert_meta_dict = {
        "<![CDATA[AnatomicalStructurePrimary]]>": "<![CDATA[" + _surf_hemi + "]]>",
        "<![CDATA[AnatomicalStructureSecondary]]>": "<![CDATA[" + _surf_type + "]]>",
        "<![CDATA[GeometricType]]>": "<![CDATA[" + _geo_type + "]]>",
        "<![CDATA[Name]]>": "<![CDATA[#1]]>",
    }
...
def save_gifti_metric(metric, save_dir, surf_hemi="left", metric_type="curv"):
    ...
    metric_meta_dict = {
        "<![CDATA[Name]]>": "<![CDATA[" + _metric_type + "]]>",
        "<![CDATA[PaletteColorMapping]]>": "<![CDATA[" + cmap_meta + "]]>",
    }
...
def create_wb_spec(subj_dir):
    ...
    # add to xml tree
    et.SubElement(
        caret, "DataFile", Structure="Invalid", DataFileType="VOLUME", Selected="true"
    ).text = file_name.split("/")[-1]

    # save .spec file
    spec = et.ElementTree(caret)
    et.indent(spec)
    spec.write(subj_dir + "_wb.spec", encoding="UTF-8", xml_declaration=True)
