...
from utils.mesh import laplacian_smooth, vert_normal
...
def curvature(vert, face, curv_type="mean", smooth_iters=0):
    ...
    neigh_matrix = torch.sparse_coo_tensor(edge, values, (n_vert, n_vert, 6)).unsqueeze(
        0
    )
    # sum all neighbors
    ...
    a = (
        norm_x * (-sig_xy2 + sig_x * sig_y + sig_y * sig_y)
        - norm_xy * sig_xy * sig_y
        + norm_y * sig_xy2
    ) / denom_
    b = (
        -norm_x * sig_xy * sig_y + norm_xy * sig_x * sig_y - norm_y * sig_x * sig_xy
    ) / denom_
    c = (
        norm_x * sig_xy2
        - norm_xy * sig_x * sig_xy
        + norm_y * (sig_x * sig_x - sig_xy2 + sig_x * sig_y)
    ) / denom_
    trC = a + c
    detC = a * c - b * b
    temp = trC * trC - 4 * detC
    ...
    k1[torch.where(denom == 0)] = 0.0
    k2[torch.where(denom == 0)] = 0.0
    k1[torch.where(temp < 0)] = 0.0
    k2[torch.where(temp < 0)] = 0.0
    ...
    curv = laplacian_smooth(curv.unsqueeze(-1), face, lambd=1.0, n_iters=smooth_iters)
...
def sulcal_depth(vert, face, verbose=False):
    ...
    _, sulc = mris_inflate(vert, face, track_sulcal_depth=True, verbose=verbose)
    return sulc


def myelin_map(
    subj_dir, surf_hemi="left", myelin_sigma=5 / (2 * (np.sqrt(2 * np.log(2))))
):
    """
    ...
    subprocess.run(
        ...
        + "_hemi-"
        + surf_hemi
        + "_thickness.shape.gii "
        + str(myelin_sigma),
        shell=True,
    )

    # remove the gifti file, and save it later to change colormap
    myelin = nib.load(subj_dir + "_hemi-" + surf_hemi + "_myelinmap.shape.gii")
    myelin = myelin.agg_data()
    os.remove(subj_dir + "_hemi-" + surf_hemi + "_myelinmap.shape.gii")
    return myelin


def smooth_myelin_map(
    subj_dir, surf_hemi="left", smooth_sigma=4 / (2 * (np.sqrt(2 * np.log(2))))
):
    """
    ...
    os.remove(subj_dir + "_hemi-" + surf_hemi + "_smoothed_myelinmap.shape.gii")
