...
def wb_inflate(
    vert, vert0, face, strength=1.0, n_iters=1, n_cycles=1, inflation_factor=1.0
):
    """
    ...
    vert = vert - v_center
    scale_factor = inflation_factor - 1.0
    for c in range(n_cycles):
        # surface smooth
        vert = area_weighted_smooth(vert, face, strength=strength, n_iters=n_iters)
        # surface inflation
        radius = (vert / v_range).norm(dim=-1)
        scale = 1.0 + scale_factor * (1.0 - radius)
...
def generate_inflated_surfaces(vert, face, iter_scale=1.0):
    ...
    vert0 = vert.clone()
    low_smooth = wb_inflate(
        vert,
        vert0,
        face,
        strength=0.2,
        n_iters=int(50 * iter_scale),
        n_cycles=1,
    ...
    inflated = wb_inflate(
        ...
        face,
        strength=1.0,
        n_iters=int(30 * iter_scale),
        n_cycles=2,
        inflation_factor=1.4,
    )
    very_inflated = wb_inflate(
        inflated,
        vert0,
        face,
        strength=1.0,
...
def mris_inflate(
    ...
    dist_0 = (vert[0, edge_n[1]] - vert[0, edge_n[0]]).norm(dim=-1, keepdim=True)
    ...
    n_avgs = [2**i for i in range(max_level - 2, np.maximum(min_level, 1) - 2, -1)]
    if min_level == 0:
        n_avgs.append(0)
    disp = 0
    ...
    for level in range(len(n_avgs)):
        for t in range(n_steps):
            ...
            dist_t = (vert_t[0, edge_n[1]] - vert_t[0, edge_n[0]]).norm(
                dim=-1, keepdim=True
            )
            ...
            distort = torch.sparse_coo_tensor(
                edge_n, (dist_t - dist_0 / scale_t) / dist_t * e_t, (n_vert, n_vert, 3)
            ).unsqueeze(0)
            grad_distort = torch.sparse.sum(distort, dim=-2).to_dense() / degree_n
            ...
            disp_scale = torch.min(
                torch.ones_like(disp_norm), max_dx / disp_norm
            ).unsqueeze(-1)
            ...
            if verbose:
                ...
                Delta2 = torch.sparse_coo_tensor(
                    edge_n, delta2, (n_vert, n_vert)
                ).unsqueeze(0)
                J_distort = torch.sparse.sum(Delta2, dim=-1).to_dense().unsqueeze(-1)
                J_distort = (J_distort / degree_n).mean().item()
                J = (
                    w_inflate * J_inflate
                    + w_distort * np.sqrt(n_avgs[level]) * J_distort
                )
                print(
                    "Level:{}, Iter:{}, RMSE:{}, Energy:{}, Inflation:{}, Distortion:{}.".format(
                        level + 1,
                        t + 1,
                        np.round(rmse, 5),
                        np.round(J, 5),
                        np.round(J_inflate, 5),
    ...
    vert_center = (vert_t.max(1, keepdim=True)[0] + vert_t.min(1, keepdim=True)[0]) / 2
