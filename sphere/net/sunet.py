...
from sphere.net.layers import conv_block, conv_layer, pool_layer, upconv_layer

from sphere.net.interp import sphere_interpolate, get_bi_interp

from sphere.net.utils import get_neighs_order, get_upconv_index
...
class SUNet(nn.Module):
    ...
    def __init__(
        ...
        self.up4 = upconv_layer(
            C_hid[4], C_hid[4], upconv_top_index[3], upconv_down_index[3]
        )
        self.up3 = upconv_layer(
            C_hid[3], C_hid[3], upconv_top_index[2], upconv_down_index[2]
        )
        self.up2 = upconv_layer(
            C_hid[2], C_hid[2], upconv_top_index[1], upconv_down_index[1]
        )
        self.up1 = upconv_layer(
            C_hid[1], C_hid[1], upconv_top_index[0], upconv_down_index[0]
        )
...
class SphereDeform(nn.Module):
    ...
    def __init__(self, C_in=3, C_hid=[32, 64, 128, 256, 512], device="cpu"):
        ...
        vert_fixed = nib.load(
            "./template/sphere_" + str(n_vert) + ".surf.gii"
        ).agg_data("pointset")
        ...
        self.msunet = SUNet(
            C_in, C_hid, self.neigh_orders, upconv_top_index, upconv_down_index
        ).to(device)
    ...
    def forward(self, feat_in, vert_in, n_steps=7):
        ...
        phi = phi[:, self.neigh_orders[0]].reshape(-1, phi.shape[1], 7, 3).mean(-2)
