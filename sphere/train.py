...
from utils.mesh import face_area, apply_affine_mat, taubin_smooth
...
class SphereDataset(Dataset):
    ...
    def __init__(self, args, data_split="train"):
        ...
        # ------ load template input ------
        img_temp = nib.load("./template/dhcp_week-40_template_T2w.nii.gz")
        surf_temp = nib.load(
        ...
        vert_temp = apply_affine_mat(vert_temp, np.linalg.inv(affine_temp))
        face_temp = face_temp[:, [2, 1, 0]]

        # ------ for sphere interpolation ------
        # load pre-computed barycentric coordinates
        barycentric = nib.load(
            "./template/dhcp_week-40_hemi-" + surf_hemi + "_barycentric.gii"
        )
        ...
        nn_surf.load_state_dict(torch.load(model_dir, map_location=device))
        ...
        for i in tqdm(range(len(subj_list))):
            ...
            # ------ load input volume ------
            vol_in = nib.load(subj_dir + "/" + subj_id + "_T2w_proc_affine.nii.gz")
...
def train_loop(args):
    ...
    face_in = torch.LongTensor(face_in[None]).to(device)
    edge_in = torch.cat(
        [face_in[0, :, [0, 1]], face_in[0, :, [1, 2]], face_in[0, :, [2, 0]]], dim=0
    ).T
    ...
    # ------ load model ------
    nn_sphere = SphereDeform(C_in=6, C_hid=[32, 64, 128, 256, 256], device=device)
    ...
    for epoch in tqdm(range(n_epoch + 1)):
        ...
        for idx, data in enumerate(trainloader):
            ...
            optimizer.zero_grad()
            vert_sphere_pred = nn_sphere(feat_160k, vert_sphere_in, n_steps=7)

            # unsupervised metric distortion loss
            edge_loss = edge_distortion(vert_sphere_pred, vert_wm_in, edge_in)
            area_loss = area_distortion(vert_sphere_pred, vert_wm_in, face_in)
        ...
        if epoch % 10 == 0:  # start validation
            ...
            torch.save(
                nn_sphere.state_dict(),
                "./sphere/ckpts/model_hemi-"
                + surf_hemi
                + "_sphere_"
                + tag
                + "_"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n_epoch", default=100, type=int, help="number of training epochs"
    )
    parser.add_argument(
        "--w_edge", default=1.0, type=float, help="weight for edge distortion loss"
    )
    parser.add_argument(
        "--w_area", default=0.5, type=float, help="weight for area distortion loss"
    )
