...
from utils.mesh import apply_affine_mat, adjacent_faces, taubin_smooth, face_normal
...
class SurfDataset(Dataset):
    ...
    def __init__(self, args, data_split="train"):
        ...
        for i in tqdm(range(len(subj_list))):
            ...
            vert_gt = apply_affine_mat(vert_gt, np.linalg.inv(affine_in)).astype(
                np.float32
            )
...
def train_loop(args):
    ...
    logging.basicConfig(
        filename="./surface/ckpts/log_hemi-"
        + surf_hemi
        + "_"
        + surf_type
        + "_"
    ...
    surf_temp = nib.load("./template/dhcp_week-40_hemi-" + surf_hemi + "_init.surf.gii")
    ...
    nn_surf = SurfDeform(
        C_hid=C_hid, C_in=1, inshape=[112, 224, 160], sigma=sigma, device=device
    )
    ...
    for epoch in tqdm(range(n_epoch + 1)):
        ...
        for idx, data in enumerate(trainloader):
            ...
            optimizer.zero_grad()
            vert_pred = nn_surf(vert_in, vol_in, n_steps=7)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=1.0,
        type=float,
        help="standard deviation for gaussian smooth",
    )
    parser.add_argument(
        "--w_nc", default=3.0, type=float, help="weight for normal consistency loss"
    )
    parser.add_argument(
        "--w_edge", default=0.3, type=float, help="weight for edge length loss"
    )
