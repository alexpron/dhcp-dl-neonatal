...
from utils.io import Logging, save_numpy_to_nifti, save_gifti_surface
...
def split_data(orig_dir, save_dir, seed=12345):
    ...
    subj_list = sorted(glob.glob(orig_dir +
                                 "sub-*/ses-*/haste/*desc-aligned_T2w.nii.gz"))
...
def process_data(orig_dir, save_dir):
    ...
    img_fix_ants = ants.image_read("./template/dhcp_week-40_template_T2w.nii.gz")
    affine_fix = nib.load("./template/dhcp_week-40_template_T2w.nii.gz").affine
    ...
    for n in range(3):
        ...
        for i in tqdm(range(len(subj_list))):
            ...
            subj_orig_dir = (
                orig_dir + subid + "/" + sesid + "/anat/" + subid + "_" + sesid
            )
            subj_save_dir = (
                save_dir
                + data_split[n]
            ...
            #img_restore_nib = nib.load(subj_orig_dir +
            # "_desc-restore_T2w.nii.gz")
            # brainmask by bet
            ...
            path_t2_brain_extracted = ''
            img_proc_nib = nib.load(path_t2_brain_extracted)
            img_proc = img_proc_nib.get_fdata()
            affine_proc = img_proc.affine
            ...
            # save_numpy_to_nifti(
            #     ribbon.astype(np.float32), affine_orig, subj_save_dir + "_ribbon.nii.gz"
            # )
            ...
            # save_numpy_to_nifti(
            #     img_orig_down.astype(np.float32),
            #     affine_orig,
            #     subj_save_dir + "_T2w_orig_down.nii.gz",
            # )
            save_numpy_to_nifti(
                img_proc_down.astype(np.float32),
                affine_proc,
            ...
            img_move_ants = ants.image_read(path_t2_brain_extracted)
            ...
            img_align_ants, affine_align, _, _, align_dice = registration(
                img_move_ants, img_fix_ants, affine_fix, out_prefix=subj_save_dir
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="./dhcp_dir/",
        type=str,
        help="directory of original dHCP dataset",
    )
    parser.add_argument(
        "--save_dir",
        default="./data/",
